...
def download_sura(server_url, surah, path):
...
def download(recitation):
    ...
    for surah_number in xrange(1, 115):
        download_sura(server, surah_number, directory_path)
...
def print_listing(recitations):
    for id, recitation in recitations.iteritems():
...
def main():
    ...
    if not requested_recitation_id:
        ...
        if set(sys.argv).intersection(['--list', '-l']):
            sys.exit()
        requested_recitation_id = raw_input(PROMPT)
